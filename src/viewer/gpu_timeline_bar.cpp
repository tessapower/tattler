...
static ImVec4 EventColor(EventType type)
{
    ...
    switch (type)
    {
    case EventType::Draw:            return p.pine;
    case EventType::DrawIndexed:     return p.foam;
    case EventType::Dispatch:        return p.iris;
    case EventType::ResourceBarrier: return p.gold;
    case EventType::ClearRTV:        return p.rose;
    case EventType::ClearDSV:        return p.love;
    case EventType::CopyResource:    return p.muted;
    case EventType::Present:         return p.subtle;
    default:                         return p.hlHigh;
    }
}
...
static const char* EventTypeName(EventType type)
{
    ...
    case EventType::DrawIndexed:     return "DrawIndexed";
    case EventType::Dispatch:        return "Dispatch";
    case EventType::ResourceBarrier: return "ResourceBarrier";
    case EventType::ClearRTV:        return "ClearRTV";
    case EventType::ClearDSV:        return "ClearDSV";
    case EventType::CopyResource:    return "CopyResource";
    case EventType::Present:         return "Present";
    default:                         return "Unknown";
    }
}

void GpuTimelineBar::Draw(const CaptureSnapshot* snapshot, const CapturedEvent* selectedEvent)
{
    ...
    for (const auto& frame : snapshot->frames)
    {
        ...
        for (const auto& event : frame.events)
        {
            float x0 = barOrigin.x +
                       static_cast<float>(event.timestampBegin - minTs) / totalRange * barWidth;
            float x1 = barOrigin.x +
                       static_cast<float>(event.timestampEnd - minTs) / totalRange * barWidth;
            ...
            drawList->AddRectFilled({x0, barOrigin.y}, {x1, barOrigin.y + rowHeight - 2.0f},
                                    ImGui::ColorConvertFloat4ToU32(col), 2.0f);

            if (selectedEvent == &event)
            {
                const Palette& p = GetCurrentPalette();
                drawList->AddRect({x0, barOrigin.y}, {x1, barOrigin.y + rowHeight - 2.0f},
                                  ImGui::ColorConvertFloat4ToU32(p.love), 2.0f, 0, 2.0f);
            }

            if (ImGui::IsMouseHoveringRect({x0, barOrigin.y}, {x1, barOrigin.y + rowHeight - 2.0f}))
            {
                double ms = frame.gpuFrequency > 0
                    ? static_cast<double>(event.timestampEnd - event.timestampBegin) /
                      static_cast<double>(frame.gpuFrequency) * 1000.0
                    : 0.0;
                ImGui::SetTooltip("%s\nFrame %u · Event %u\n%.3f ms",
                                  EventTypeName(event.type),
                                  event.frameIndex, event.eventIndex, ms);
            }
        }

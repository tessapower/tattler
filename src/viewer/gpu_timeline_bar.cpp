                          ...
                          const CapturedEvent* selectedEvent)
{
    ...
    m_selectedFrameIndex = std::clamp(m_selectedFrameIndex, 0, static_cast<int>(snapshot->frames.size()) - 1);
    ...
    ImGui::SameLine();
    ImGui::Text("Frame %u / %zu", currentFrame.frameNumber, snapshot->frames.size());
    ImGui::SameLine();
    if (ImGui::Button("Next >>"))
    {
        m_selectedFrameIndex = std::min(static_cast<int>(snapshot->frames.size()) - 1, m_selectedFrameIndex + 1);
        m_scrollOffset = 0.0f; // Reset scroll when changing frames
    }
    ImGui::SameLine();
    ...
    ImGui::SameLine();
    if (ImGui::Button("Fit Frame"))
    {
        // Zoom to 10000% to see fine detail
        m_zoomLevel = 100.0f;
        m_scrollOffset = 0.0f;
    }
    ImGui::Separator();

    // Find the timestamp range for the current frame only
    ...
    if (ImGui::IsWindowHovered())
    {
        ...
        if (wheel != 0.0f)
        {
            ...
            float relativeX = (mousePos.x - windowPos.x) / ImGui::GetWindowWidth();
            m_scrollOffset += (m_zoomLevel - oldZoom) * relativeX * ImGui::GetContentRegionAvail().x;
        }
    }
    ...
    for (const auto& event : currentFrame.events)
    {
        float x0 =
            barOrigin.x + static_cast<float>(event.timestampBegin - minTs) /
                              totalRange * zoomedWidth - m_scrollOffset;
        float x1 =
        ...
        if (ImGui::IsMouseHoveringRect(
                {x0, barOrigin.y}, {x1, barOrigin.y + rowHeight - 2.0f}))
        {
            double ms = currentFrame.gpuFrequency > 0
